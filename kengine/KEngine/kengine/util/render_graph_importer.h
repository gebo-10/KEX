...
namespace kengine {
	...
	public:
		...
		static RenderGraphPtr import(const string & filename) {
			...
			std::vector<Attachment> ats;
			TextureDesc desc;
			desc.width = 1024;
			desc.height = 1024;
			desc.internal_format = TextureInternalFormat::RGBA8;
			auto texture = std::make_shared<Texture>(desc);
			ats.push_back(Attachment(AttachmentPoint::COLOR_ATTACHMENT0, texture));
			auto rbo = std::make_shared<RenderBuffer>(1024, 1024, RenderBufferFormat::DEPTH_COMPONENT);
			ats.push_back(Attachment(AttachmentPoint::DEPTH,rbo ));
			auto render_target = std::make_shared<RenderTarget>(std::move(ats));
			dp2->target = render_target;
...

			{
				...
				std::vector<Attachment> ats;
				auto texture_x = std::make_shared<Texture>(desc);
				ats.push_back(Attachment(AttachmentPoint::COLOR_ATTACHMENT0, texture_x));
				auto blur_x_render_target = std::make_shared<RenderTarget>(std::move(ats));

				std::vector<Attachment> ats2;
				auto texture_y = std::make_shared<Texture>(desc);
				ats2.push_back(Attachment(AttachmentPoint::COLOR_ATTACHMENT0, texture_y));
				auto blur_y_render_target = std::make_shared<RenderTarget>(std::move(ats2));

				{
					auto material = MatGLSLImport::import("main/material/direction_blur.mg");
					material->add_texture(0, texture);
					auto dir = vec2(1, 0);
					material->set_uniform(3, glm::value_ptr(dir));
					auto blur_x = std::make_shared<BlitPass>(blur_x_render_target, material);
					render_graph->passes.push_back(blur_x);
				}
				{
					auto material = MatGLSLImport::import("main/material/direction_blur.mg");
					material->add_texture(0, texture_x);
					auto dir = vec2(0, 1);
					material->set_uniform(3, glm::value_ptr(dir));
					auto blur_y = std::make_shared<BlitPass>(blur_y_render_target, material);
					render_graph->passes.push_back(blur_y);
				}
			
				{
					auto material = MatGLSLImport::import("main/material/direction_blur.mg");
					material->add_texture(0, texture_y);
					auto dir = vec2(1, 0);
					material->set_uniform(3, glm::value_ptr(dir));
					auto blur_x = std::make_shared<BlitPass>(blur_x_render_target, material);
					render_graph->passes.push_back(blur_x);
