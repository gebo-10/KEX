...
namespace kengine {
	...
	public:
		...
		Cube()
		{
			auto pos_buf = std::make_shared<Buffer>(vertices, sizeof(vertices));
			set_position(pos_buf);

			auto index_buf = std::make_shared<Buffer>(index, sizeof(index));
			set_indices(index_buf);
